...
def optimize_lineups(my_roster, their_roster, my_stats, their_stats):
    ...
    total_cost = 0
    print("")
    team = common.build_full_team("rob_roster.csv", zstats_csv_source)
    my_cats = calc_best_cat_totals(team)
    print("")
    print("Kyle")
    team = common.build_full_team("kyle_roster.csv", zstats_csv_source)
    cats = calc_best_cat_totals(team)
    total_cost += calc_cost(my_cats, cats)
    print("")
    print("Ben")
    team = common.build_full_team("ben_roster.csv", zstats_csv_source)
    cats = calc_best_cat_totals(team)
    total_cost += calc_cost(my_cats, cats)
    print("")
    print("Dylan")
    team = common.build_full_team("dylan_roster.csv", zstats_csv_source)
    cats = calc_best_cat_totals(team)
    total_cost += calc_cost(my_cats, cats)
    print("")
    print("George")
    team = common.build_full_team("george_roster.csv", zstats_csv_source)
    cats = calc_best_cat_totals(team)
    total_cost += calc_cost(my_cats, cats)
    print("")
    print("Alex")
    team = common.build_full_team("alex_roster.csv", zstats_csv_source)
    cats = calc_best_cat_totals(team)
    total_cost += calc_cost(my_cats, cats)
    print("")
    print("Akbar")
    team = common.build_full_team("akbar_roster.csv", zstats_csv_source)
    cats = calc_best_cat_totals(team)
    total_cost += calc_cost(my_cats, cats)
    print("")
    print("Tom")
    team = common.build_full_team("tom_roster.csv", zstats_csv_source)
    cats = calc_best_cat_totals(team)
    total_cost += calc_cost(my_cats, cats)
    print("")
    print("Brandt")
    team = common.build_full_team("brandt_roster.csv", zstats_csv_source)
    cats = calc_best_cat_totals(team)
    total_cost += calc_cost(my_cats, cats)
    playoff_wins += calc_cost(my_cats, cats)
    print("")
    print("ZMo")
    team = common.build_full_team("zmo_roster.csv", zstats_csv_source)
    cats = calc_best_cat_totals(team)
    total_cost += calc_cost(my_cats, cats)
    playoff_wins += calc_cost(my_cats, cats)
    print("")
